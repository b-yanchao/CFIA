    ...
    def forward(self, x):
        ...
        for b in range(batch_size):
            for c in range(channels):
                # 将特定批次的特定通道特征图移动到CPU并转为numpy数组
                x_np = x[b, c, :, :].detach().cpu().numpy()
                # 使用pywavelets进行2D小波变换
...
class HighPassWithMaxPool(nn.Module):
    def __init__(self, in_channels):
        ...
        self.high_pass_conv.weight.requires_grad = False  # 固定滤波器参数不训练

        # 初始化一个3x3的高通卷积核
        # 一个常见的高通卷积核示例，注意力放在中心像素上，与周围像素差异化
...
def sub_pixel_convolution(input_tensor, upscale_factor=2):
    ...
    参数:
    - input_tensor: 输入的特征图张量，预期维度为 (N, C, H, W)
    - upscale_factor: 上采样的因子，例如2表示将各维度尺寸放大2倍
